...
class ListingReviewController extends Controller
{
    ...
    public function start(Request $request, Property $property): JsonResponse
    {
        return response()->json([
            ...
            'data' => $this->data($this->workflow->startReview($request->user(), $property, $request), false),
        ]);
    }

    public function returnForCorrection(Request $request, Property $property): JsonResponse
    {
        $v = $request->validate(['reason' => ['required', 'string', 'min:5', 'max:2000']]);
        return response()->json([
            'message' => 'Listing returned for correction.',
            'data' => $this->data(
                $this->workflow->returnForCorrection($request->user(), $property, $v['reason'], $request),
                false,
            ),
        ]);
    }
    ...
    public function approve(Request $request, Property $property): JsonResponse
    {
        $v = $request->validate([
            ...
            'duplicate_review_reason' => ['nullable', 'string', 'max:2000'],
        ]);
        return response()->json([
            ...
            'data' => $this->data(
                $this->workflow->approve(
                    ...
                    $v['reason'] ?? null,
                    $request,
                    $v['duplicate_review_reason'] ?? null,
                ),
                false,
            ),
        ]);
    ...
    public function rejectFinal(Request $request, Property $property): JsonResponse
    {
        $v = $request->validate(['reason' => ['required', 'string', 'min:10', 'max:3000']]);
        return response()->json([
            'message' => 'Physical property blocked for this publication purpose.',
            'data' => $this->data(
                $this->workflow->finalRejectAndBlock($request->user(), $property, $v['reason'], $request),
                false,
            ),
        ]);
    }

    public function linkPropertyAsset(Request $request, Property $property): JsonResponse
    {
        $v = $request->validate([
            'property_asset_id' => ['required', 'integer', 'exists:property_assets,id'],
            'reason' => ['required', 'string', 'min:5', 'max:2000'],
        ]);
        $asset = PropertyAsset::query()->findOrFail((int) $v['property_asset_id']);
        return response()->json([
            'message' => 'Listing linked to physical property identity.',
            'data' => $this->data(
                $this->workflow->linkAsset($request->user(), $property, $asset, $v['reason'], $request),
                false,
            ),
        ]);
    }
    ...
    public function blocks(Request $request): JsonResponse
    {
        ...
        $q = PropertyPublicationBlock::query()->with(['propertyAsset', 'blockedBy', 'liftedBy'])->latest('id');
        if (array_key_exists('active', $v)) {
            $q->where('is_active', (bool) $v['active']);
        }
        $page = $q->paginate((int) ($v['per_page'] ?? 50));
        return response()->json([
            'data' => collect($page->items())->map(fn ($b) => $this->blockData($b))->values(),
            'meta' => [
                'current_page' => $page->currentPage(),
                'last_page' => $page->lastPage(),
                'total' => $page->total(),
    ...
    public function liftBlock(Request $request, PropertyPublicationBlock $block): JsonResponse
    {
        $v = $request->validate(['reason' => ['required', 'string', 'min:10', 'max:3000']]);
        return response()->json([
            'message' => 'Publication block lifted.',
            'data' => $this->blockData($this->workflow->liftBlock($request->user(), $block, $v['reason'], $request)),
        ]);
    }

    public function document(Request $request, ListingDocument $document)
    {
        $document->loadMissing('property');
        $user = $request->user();
        $isOwner = $document->property && (int) $document->property->user_id === (int) $user->id;
        abort_unless($isOwner || $user->hasPermission('listings.moderate'), 403);
        abort_unless(app(\App\Services\CloudAssetStorageService::class)->existsPrivate($document->path), 404);

        $this->audit->record(
            $user,
            'listing.proof_document_viewed',
            $document,
            ['listing_id' => $document->property_id, 'kind' => $document->kind],
            $request,
            $document->property?->user_id,
        );
        return app(\App\Services\CloudAssetStorageService::class)->downloadPrivate(
            $document->path,
    ...
    {
        $p->loadMissing(['user.accountVerificationProfile.documents', 'propertyAsset', 'documents', 'images']);
        $block = $p->propertyAsset?->publicationBlocks()
            ->where('purpose', $p->purpose)
            ->where('is_active', true)
            ->latest('id')
            ->first();
        $profile = $p->user?->accountVerificationProfile;
        $likelyDuplicates = in_array($p->review_status, ['submitted', 'under_review'], true)
            ? $this->assets->likelyDuplicates($p)
            : [];
        ...
        $data = [
            ...
            'likely_duplicate_count' => count($likelyDuplicates),
            'owner' => [
                'id' => $p->user?->id,
                'name' => $p->user?->name,
                'email' => $p->user?->email,
            ...
            'images' => $p->images->map(fn ($image) => [
                'id' => $image->id,
                'url' => '/api/property-media/' . $image->id,
                'is_primary' => (bool) $image->is_primary,
                'sort_order' => (int) $image->sort_order,
            ])->values(),
            'proof_documents' => $p->documents->map(fn ($d) => [
                'id' => $d->id,
                'kind' => $d->kind,
                'original_name' => $d->original_name,
                'mime_type' => $d->mime_type,
                'size_bytes' => $d->size_bytes,
                'url' => '/api/listing-documents/' . $d->id,
            ])->values(),
        ...
        if ($full) {
            $p->loadMissing('reviews.actor');
            $data['likely_duplicate_candidates'] = $likelyDuplicates;
            $data['review_history'] = $p->reviews->map(fn ($r) => [
                'id' => $r->id,
                'action' => $r->action,
                'from' => $r->from_review_status,
                'to' => $r->to_review_status,
                'reason' => $r->reason,
            ...
            $data['area_m2'] = $p->area_m2;
            $data['area_value'] = $p->area_value !== null ? (float) $p->area_value : null;
            $data['area_unit'] = $p->area_unit;
            $data['bedrooms'] = $p->bedrooms;
            $data['bathrooms'] = $p->bathrooms;
            $data['has_parking'] = $p->has_parking;
            $data['building_facade'] = $p->building_facade;
        }
    ...
    private function blockData(PropertyPublicationBlock $b): array
    {
        return [
            'id' => $b->id,
            ...
            'lifted_by_user_id' => $b->lifted_by_user_id,
            'lifted_reason' => $b->lifted_reason,
