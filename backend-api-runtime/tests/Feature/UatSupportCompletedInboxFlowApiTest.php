...
class UatSupportCompletedInboxFlowApiTest extends TestCase
{
    ...
    public function test_waiting_user_is_not_returned_as_unassigned_inbox_work(): void
    {
        ...
        $teamId=$this->putAgentInFallback($agent);
        $task=SupportTask::query()->create([
            'source_type'=>'account_verification','source_id'=>999,'subject'=>'تحقق حساب مالك','status'=>'waiting_user','priority'=>'normal',
            'support_team_id'=>$teamId,'assigned_to_user_id'=>$agent->id,'assigned_to_name_snapshot'=>$agent->name,'last_activity_at'=>now(),
        ]);
    ...
    public function test_completed_scope_keeps_executor_and_listing_return_result(): void
    {
        ...
        $task=SupportTask::query()->create([
            ...
            'support_team_id'=>$teamId,'assigned_to_user_id'=>$agent->id,'assigned_to_name_snapshot'=>$agent->name,
            'claimed_at'=>now()->subMinutes(5),'completed_at'=>now(),'last_activity_at'=>now(),
            'metadata'=>['review_status'=>'returned_for_correction','resolution'=>'returned_for_correction'],
        ]);
    ...
    public function test_manager_assignment_notification_mentions_my_tasks(): void
    {
        ...
        [, $managerHeaders]=$this->user('assigned-manager@example.test','+967744420004',['support_manager']);
        $teamId=$this->putAgentInFallback($agent);
        $task=SupportTask::query()->create([
            'source_type'=>'support_ticket','source_id'=>777,'subject'=>'تذكرة مسندة','status'=>'new','priority'=>'normal',
            'support_team_id'=>$teamId,'last_activity_at'=>now(),
        ]);
        $this->withHeaders($managerHeaders)->putJson('/api/admin/workspace/tasks/'.$task->id.'/assign',['user_id'=>$agent->id])
            ->assertOk()->assertJsonPath('data.assigned_to_user_id',$agent->id);
        $this->assertDatabaseHas('user_notifications',[
            'user_id'=>$agent->id,'type'=>'support_task_assigned','title'=>'قام مدير الدعم بإسناد مهمة لك',
        ]);
    }

    private function putAgentInFallback(User $agent): int
    {
        $fallback=(int)DB::table('support_teams')->where('is_fallback',true)->value('id');
        DB::table('support_team_members')->updateOrInsert(
            ['support_team_id'=>$fallback,'user_id'=>$agent->id],
            ['member_role'=>'agent','is_available'=>true,'capacity'=>10,'joined_at'=>now(),'created_at'=>now(),'updated_at'=>now()],
        );
        return $fallback;
    }
    ...
    private function user(string $email,string $phone,array $roles=[]): array
    {
        $user=User::query()->create([
            'name'=>'Support Flow User','email'=>$email,'phone'=>$phone,'phone_verified_at'=>now(),
        ...
        $user->roles()->sync($ids);
        $plain='sf_'.substr(hash('sha512',$email),0,80);
        $user->apiTokens()->create([
            'name'=>'support-flow-test','token_hash'=>hash('sha256',$plain),'token_prefix'=>substr($plain,0,12),'expires_at'=>now()->addHour(),
        ]);
